...
export default function companyAddEmployeeValidation(values: IAddEmployeeInfo): IAddMembersErrorInfo {
  ...
  if (!/^(?=\S)(?!.*[^\x20-\x7E])(?=.{3,50}$)[a-zA-Z ]+$/.test(values.name)) errors.firstName = 'El nombre es obligatorio';
  ...
  if (!/^[a-zA-Z\s\-']{3,50}$/.test(values.role)) errors.jobRole = 'El puesto debe tener entre 3 y 50 caracteres y puede incluir letras, espacios, guiones y apóstrofes';
